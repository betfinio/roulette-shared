...
export function handleRouletteNumberRolled(event: BetEndedEvent): void {
  ...
  if (!singlePlayerAddress.equals(event.address)) {
  ...
  rouletteNumber.lastUpdated = event.block.timestamp;
  const countDecimal = rouletteNumber.count.toBigDecimal();
  // Update the score
...
export function handleRouletteStats(event: BetEndedEvent): void {
  ...
  const isZero = rolledNumber === 0;
  const isEven = rolledNumber !== 0 && rolledNumber % 2 === 0;
  const isOdd = rolledNumber !== 0 && rolledNumber % 2 === 1;
